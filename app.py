...
def cadastrar_cliente():
    ...
    try:
        ...
        return (
            jsonify(
                {"message": "Erro: Telefone já cadastrado. O telefone deve ser único."}
            ),
            409,
        )
        ...
        return (
            jsonify(
                {"message": "Ocorreu um erro ao cadastrar o cliente.", "error": str(e)}
            ),
            500,
        )
    finally:
...
# Rota para buscar e listar clientes
...
def listar_clientes():
    ...
    clientes_formatados = []
    try:
        # Verifica se há uma query de busca com caracteres (ignora espaços em branco)
        if query and query.strip():
            search_pattern = f"%{query}%"
            cursor.execute(
                "SELECT nome, email, telefone FROM clientes WHERE nome ILIKE %s OR telefone LIKE %s;",
                (search_pattern, search_pattern),
            )
        else:
            # Se a query estiver vazia ou com espaços, lista todos os clientes
        ...
        return jsonify({"message": f"Erro na consulta SQL: {str(e)}"}), 500
    finally:
        cursor.close()
        conn.close()
...
def deletar_cliente(telefone):
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "message": "Não foi possível excluir cliente. Verifique se ele possui empréstimos ativos."
                }
            ),
            409,
        )
        ...
        return (
            jsonify(
                {"message": "Ocorreu um erro ao excluir o cliente.", "error": str(e)}
            ),
            500,
        )
...
def deletar_emprestimo(id):
    ...
    try:
        ...
        return (
            jsonify(
                {"message": "Ocorreu um erro ao excluir o empréstimo.", "error": str(e)}
            ),
            500,
        )
...
def atualizar_detalhes_emprestimo(id):
    ...
    try:
        cursor.execute(
            "UPDATE emprestimos SET detalhes = %s WHERE id = %s", (detalhes, id)
        )
        conn.commit()
        ...
        if cursor.rowcount == 0:
            return (
                jsonify(
                    {"message": "Empréstimo não encontrado ou detalhes não alterados."}
                ),
                404,
            )

        return (
            jsonify({"message": "Detalhes do empréstimo atualizados com sucesso!"}),
            200,
        )
        ...
        return (
            jsonify(
                {
                    "message": "Ocorreu um erro ao atualizar os detalhes do empréstimo.",
                    "error": str(e),
                }
            ),
            500,
...
def buscar_cliente(telefone):
    ...
    try:
        cursor.execute(
            "SELECT nome, email, telefone FROM clientes WHERE telefone = %s",
            (telefone,),
        )
        ...
        return (
            jsonify(
                {"message": "Ocorreu um erro ao buscar o cliente.", "error": str(e)}
            ),
            500,
        )
    finally:
...
def atualizar_cliente(telefone):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            return (
                jsonify({"message": "Cliente não encontrado ou dados não alterados."}),
                404,
            )
        ...
        return (
            jsonify(
                {"message": "Ocorreu um erro ao atualizar o cliente.", "error": str(e)}
            ),
            500,
        )
    finally:
...
def cadastrar_emprestimo():
    ...
    if not all(
        [valor_emprestado, juros_mensal, num_meses, cliente_telefone, valor_parcela]
    ):
    ...
    try:
        ...
        return (
            jsonify(
                {"message": "Empréstimo cadastrado com sucesso!", "id": emprestimo_id}
            ),
            201,
        )
    except psycopg2.IntegrityError:
        ...
        return (
            jsonify(
                {
                    "message": "Ocorreu um erro ao cadastrar o empréstimo.",
                    "error": str(e),
                }
            ),
            500,
...
def listar_emprestimos_cliente(telefone):
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "message": "Ocorreu um erro ao buscar os empréstimos.",
                    "error": str(e),
                }
            ),
            500,
